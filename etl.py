...
def process_log_file(cur, filepath):
    ...
    for i in range(len(time_data)):
        dct[column_labels[i]] = time_data[i]
    time_df = pd.DataFrame.from_dict(dct)
    ...
    frame = {'userId': df.loc[:,'userId'],
         'firstName': df.loc[:,'firstName'],
         'lastName': df.loc[:,'lastName'],
         'gender': df.loc[:,'gender'],
         'level': df.loc[:,'level']
        }
    
    user_df = pd.DataFrame(frame)
...
def process_data(cur, conn, filepath, func):
    """
    - get all files
    - iterate over files and process and inform
    """
...
def main():
    """
    - main function to reach the other functions
    """
